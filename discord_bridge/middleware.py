...
MiddlewareFunction = Callable[[SmartMessage, Callable[[SmartMessage], Awaitable[None]]], Awaitable[None]]
...
class MiddlewareManager:
    ...
    async def execute(
        self, 
        message: SmartMessage, 
        handler: Callable[[SmartMessage], Awaitable[None]]
    ) -> bool:
...
async def logging_middleware(ctx: MiddlewareContext, next: Callable) -> None:
    ...
    """
    logger.info(f"[START] Processing command from {ctx.message.author_name}: {ctx.message.content}")
    await next()
...
async def rate_limit_middleware(
    ...
    rate_limit_middleware._user_requests[user_id] = [
        req_time for req_time in rate_limit_middleware._user_requests[user_id]
