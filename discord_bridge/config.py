...
class BridgeConfig(BaseModel):
    ...
    discord_token: str = Field(
        ..., description="Discord bot token from Discord Developer Portal", min_length=1
    )
    command_prefix: str = Field(
        default="!", description="Prefix for bot commands", min_length=1, max_length=10
    )
    allowed_channel_ids: List[int] = Field(
        default_factory=list, description="List of whitelisted Discord channel IDs"
    )
    ...
    def token_not_placeholder(cls, v):
        ...
        if v == "YOUR_DISCORD_BOT_TOKEN_HERE":
            raise ValueError(
                "Please replace the placeholder token with your actual Discord bot token"
            )
        return v
